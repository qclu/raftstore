...
func (*testSMTemp) Apply(command []byte, index uint64) (interface{}, error) {
	fmt.Printf("===test raft apply index %d\n", index)
	return nil, nil
}

func (*testSMTemp) ApplyMemberChange(confChange *proto.ConfChange, index uint64) (interface{}, error) {
	fmt.Printf("===test raft member change index %d===\n", index)
	return nil, nil
}

func (*testSMTemp) Snapshot() (proto.Snapshot, error) {
	fmt.Printf("===test raft snapshot===\n")
	return nil, nil
}

func (*testSMTemp) ApplySnapshot(peers []proto.Peer, iter proto.SnapIterator) error {
	fmt.Printf("===test raft apply snapshot===\n")
	return nil
}

func (*testSMTemp) HandleFatalEvent(err *raft.FatalError) {
	fmt.Printf("===test raft fatal event===\n")
	return
}

func (*testSMTemp) HandleLeaderChange(leader uint64) {
